...
my_transform=transforms.Compose([transforms.Resize(50),transforms.ToTensor()])
...
class MyDataset(Dataset):
     ...
     def __init__(self,batch_num,mode='train',own_transform=None):
        ...
        if mode=='train':
            ...
            self.labels = []
            for i in range(15):
                print('img/train/task{}/{}/*.jpg'.format(batch_num, i))
                temp = glob.glob('img/train/task{}/{}/*.jpg'.format(batch_num, i))
                #print(temp)
                #self.imgs.extend(temp[:100])
            ...
            self.labels = []
            for i in range(15):
                temp = glob.glob('img/test/task{}/{}/*.jpg'.format(batch_num, i))
                #temp = glob.glob('../testset/{}/*.png'.format(str(i)))
...
def get_multitask_experiment(name, scenario, tasks, data_dir="./datasets", only_config=False, verbose=False,
    ...
    if name == 'permMNIST':
        ...
        config = {'size': 50, 'channels': 3, 'classes': 20}
