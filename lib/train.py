...
def train_cl(model, train_datasets, test_datasets, replay_mode="none", scenario="class", classes_per_task=None, iters=2000, batch_size=32,
    ...
    # Loop over all tasks.
    for task, train_dataset in enumerate(train_datasets, 1): #enumerate(,1)中的1是指起数为1，遍历按1,2,3,...如果为0，则0,1,2,...
        ...
        output.append(precs)
        '''
        resultpath=os.path.join('./precision_onEachTask',savepath,scenario)
        modelpath=os.path.join('./models',savepath,scenario)
    ...
    #torch.save(model.state_dict(), modelpath+'/'+str(datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S'))+str(task)+'.pkl')
